...
include_once $_SERVER['DOCUMENT_ROOT'] . '/pudding-LMS-website/admin/inc/dbcon.php';
  ...
  $ntid = $_POST['ntid'];
   ...
   $nt_filename = isset($_FILES["nt_filename"]["name"]) ? $_FILES["nt_filename"]["name"] : '';
   ...
   if ($mysqli->query($sql) === true ) {
    echo "<script>
    alert('글 수정이 완료가 되었습니다.');
    location.href = 'notice_list'; </script>"; 
   } else {
    echo "<script>
    alert('글 수정 실패');
    location.href = 'notice_list'; </script>";     
  ...
  if ($result === TRUE) {    
      echo "<script>
      alert('글수정 완료되었습니다.');
      // location.replace('notice_view.php');</script>";          
  }  else {
